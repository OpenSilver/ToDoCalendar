...
partial class CalendarStoreImplementation : ICalendarStore
{
    ...
    readonly Android.Net.Uri instancesTableUri;
    ...
    readonly string[] instancesColumns = [
            ...
            CalendarContract.Instances.InterfaceConsts.EventLocation,
    ...
    public CalendarStoreImplementation()
    {
        ...
        instancesTableUri = CalendarContract.Instances.ContentUri
            ?? throw new CalendarStoreException(
                "Could not determine Android event instances table URI.");
        ...
        DateTimeOffset? endDate = null)
    {
        ...
        var uri = instancesTableUri.BuildUpon()
            ?.AppendPath(sDate.AddMilliseconds(sDate.Offset.TotalMilliseconds).ToUnixTimeMilliseconds().ToString())
            ?.AppendPath(eDate.AddMilliseconds(sDate.Offset.TotalMilliseconds).ToUnixTimeMilliseconds().ToString())
            ?.Build()
            ?? throw new CalendarStoreException("Cannot build event instances uri.");

        var calendarSpecificEvent = !string.IsNullOrEmpty(calendarId) ? $"{CalendarContract.Instances.InterfaceConsts.CalendarId} = {calendarId}" : null;
        var sortOrder = $"{CalendarContract.Instances.Begin} ASC";

        using var cursor = platformContentResolver.Query(uri,
            instancesColumns, calendarSpecificEvent, null, sortOrder)
    ...
    IEnumerable<CalendarEvent> ToEvents(ICursor cur)
    {
        while (cur.MoveToNext())
        {
            yield return ToEvent(cur);
        }
    }

    CalendarEvent ToEvent(ICursor cursor)
    {
        var timezone = cursor.GetString(cursor.GetColumnIndex(CalendarContract.Instances.InterfaceConsts.EventTimezone));
        var allDay = cursor.GetInt(cursor.GetColumnIndex(CalendarContract.Instances.InterfaceConsts.AllDay)) != 0;
        var start = DateTimeOffset.FromUnixTimeMilliseconds(cursor.GetLong(cursor.GetColumnIndex(CalendarContract.Instances.Begin)));
        var end = DateTimeOffset.FromUnixTimeMilliseconds(cursor.GetLong(cursor.GetColumnIndex(CalendarContract.Instances.End)));

        var EventIDString = cursor.GetString(cursor.GetColumnIndex(CalendarContract.Instances.EventId)) ?? string.Empty;
        ...
        return new(EventIDString,
            cursor.GetString(cursor.GetColumnIndex(CalendarContract.Instances.InterfaceConsts.CalendarId)) ?? string.Empty,
            cursor.GetString(cursor.GetColumnIndex(CalendarContract.Instances.InterfaceConsts.Title)) ?? string.Empty)
        {
            Description = cursor.GetString(cursor.GetColumnIndex(
                CalendarContract.Instances.InterfaceConsts.Description)) ?? string.Empty,
